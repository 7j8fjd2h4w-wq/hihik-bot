...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "*Условия:*\n\n"
        "Юля получает 4 хи-хика в день так как она их производит, "
        "все остальные — по 1\n\n"
        "*Команды и как всё работает:*\n\n"
        "💼 /balance — твой баланс, апгрейды и долги\n\n"
        "😂 /hihik (ответом на сообщение) — отдать хи-хик тому, кто "
        "спровоцировал это c тебя. Списывается у тебя, зачисляется ему. "
        "Если на счету есть хи-хики — трата бесплатна. Если на счету *пусто* "
        f"(берёшь \"в долг\") — обязан заплатить штраф (от {FINE_MIN_KZT} ₸) "
        f"в течение {FINE_DEADLINE_HOURS} часов\n\n"
        "🛒 /shop — магазин апгрейдов и покупки хи-хиков\n"
        f"   • /buyupgrade1 — 2 хи-хика в день вместо одного, навсегда ({UPGRADE1_COST} хи-хик)\n"
        f"   • /buyupgrade2 — разово украсть 1 хи-хик у того, кто тебя "
        f"рассмешил (в дополнение к обычному) ({UPGRADE2_COST} хи-хик)\n"
        f"   • /buyhihik <кол-во> — купить хи-хики за деньги ({HIHIK_PRICE_KZT} ₸/шт.)\n\n"
        "💸 /pay <сумма> — задекларировать оплату штрафа (ждёт подтверждения админа)\n\n"
        "🎲 Хи-хик казино — поставь хи-хики на кон:\n"
        "   • /casinocoin <ставка> — орёл/решка\n"
        "   • /casinowheel <ставка> <red/black/green> — колесо\n\n"
        "🏆 /top — таблица лидеров\n\n"
        ...
        "*Для админов:*\n"
        "/confirmpayment <ответом на сообщение или @user> <сумма> — подтвердить оплату штрафа/покупки\n"
    )
