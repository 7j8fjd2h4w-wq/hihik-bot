...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "вызвал у тебя хи-хик.\n\n"
        "* У тебя списывается 1 хи-хик, а получателю он начисляется.\n"
        "* Если на балансе есть хи-хики — списание бесплатное.\n"
        "* Если баланс пуст, ты берёшь хи-хик в долг.\n"
        ...
        "* Пока штраф не оплачен, новые хи-хики не начисляются.\n\n"
        ...
        f"* /buyupgrade2 — один раз украсть ещё 1 хи-хик у человека, который "
        f"тебя рассмешил (в дополнение к обычному переводу) ({UPGRADE2_COST} хи-хик).\n"
        f"* /buyhihik <кол-во> — купить хи-хики ({HIHIK_PRICE_KZT} ₸ за штуку).\n\n"
...
async def daily_distribution(context: ContextTypes.DEFAULT_TYPE):
    ...
    for row in all_users():
        if row["last_daily_date"] == today:
            continue
        # Пока штраф не оплачен — новые хи-хики не начисляются, но дата
        # всё равно обновляется, чтобы не начислить их задним числом позже.
        if row["debt_kzt"] and not row["debt_confirmed"]:
