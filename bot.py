...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "Юля получает 4 хи-хика в день, все остальные — по 1, потому что "
        "она продюсер хи-хика 🎬.\n\n"
        "*Команды и как всё работает:*\n\n"
        "💼 /balance — твой баланс, апгрейды и долги\n\n"
        "😂 /hihik (ответом на сообщение) — отдать хи-хик тому, кто тебя "
        "рассмешил. Списывается у тебя, зачисляется ему. Если на счету есть "
        "хи-хики — трата бесплатна. Если на счету *пусто* (берёшь \"в долг\") — "
        f"обязан заплатить штраф (от {FINE_MIN_KZT} ₸) в течение {FINE_DEADLINE_HOURS} "
        "часов, иначе все хи-хики обнулятся.\n\n"
        "💸 /pay <сумма> — задекларировать оплату штрафа (ждёт подтверждения админа)\n\n"
        "🛒 /shop — магазин апгрейдов и покупки хи-хиков\n"
        f"   • /buy_upgrade1 — 2 хи-хика в день вместо одного, навсегда ({UPGRADE1_COST} хи-хик)\n"
        f"   • /buy_upgrade2 — разово взять 2 вместо 1 ({UPGRADE2_COST} хи-хик)\n"
        f"   • /buy_hihik <кол-во> — купить хи-хики за деньги ({HIHIK_PRICE_KZT} ₸/шт.)\n\n"
        "🎲 Хи-хик казино — поставь хи-хики на кон:\n"
        ...
        "Все деньги от штрафов и покупок хи-хиков уходят на благотворительность.\n\n"
        "*Для админов:*\n"
...
async def hihik_use(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если на счету уже есть хи-хики — трата бесплатна, штраф не начисляется.
    # Штраф начисляется, только если человек берёт хи-хик "в долг" — то есть
    # у него на счету пусто (0) и он всё равно продолжает шутить.
    borrowing = actor["balance"] <= 0
    ...
    if borrowing:
        ...
        fine_note = (
            f"\n\n💸 На счету было пусто, так что это хи-хик \"в долг\"! "
            f"{name_for(actor)}, оплати штраф (минимум {FINE_MIN_KZT} ₸, можно больше) "
