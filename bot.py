...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "вызвал у тебя хи-хик. Проще всего: просто ответь на его сообщение "
        "словом «хи-хик» — команду печатать не обязательно.\n\n"
...
async def hihik_use(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    new_actor_daily_usable = actor["daily_usable"] - 1
    new_actor_balance = actor["balance"] - steal_bonus
    new_target_balance = target["balance"] + 1 + steal_bonus
    ...
    if borrowing:
        ...
        fine_note = (
            f"\n\n💸 Дневной лимит {name_for(actor)} уже закончился, так что это хи-хик "
    ...
    steal_note = (
        f"\n🕵️ {name_for(target)} применил(а) Кражу и украл(а) ещё "
        f"{steal_bonus} хи-хик из баланса {name_for(actor)} сверху!"
    ) if steal_active else ""

    total_gained = 1 + steal_bonus
    await update.message.reply_text(
        f"😂 Хи-хик засчитан!\n"
        f"{name_for(actor)} → {name_for(target)}: +{total_gained} на баланс {name_for(target)}"
        f"{steal_note}"
        f"{fine_note}"
    )
...
def main():
    ...
    # Ответ на сообщение словом "хи-хик" работает так же, как команда /hihik
    app.add_handler(MessageHandler(
        filters.REPLY & filters.Regex(r"(?i)^\s*хи[\s-]?хик\s*$"),
        hihik_use,
    ))

    # Inline-кнопки в /shop и /casino
